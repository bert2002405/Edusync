...
const SubjectsScreen = ({ navigation }) => {
  ...
  // Load subjects from AsyncStorage when component mounts
  useEffect(() => {
    loadSubjects();
  }, []);
  ...
  const loadSubjects = async () => {
    try {
      ...
      if (savedSubjects) {
        ...
        setFilteredSubjects(parsedSubjects);
      } else {
        ...
        setFilteredSubjects(defaultSubjects);
        await AsyncStorage.setItem('subjects', JSON.stringify(defaultSubjects));
      }
    } catch (error) {
      console.error('Error loading subjects:', error);
      Alert.alert('Error', 'Failed to load subjects. Please restart the app.');
    }
  };

  // Save subjects to AsyncStorage whenever they change
    ...
    try {
      ...
      if (isEditing) {
        ...
        updatedSubjects = subjects.map(s =>
          s.id === newSubject.id ? { ...subjectData, id: s.id } : s
        );
      } else {
        // Add new subject
        ...
        updatedSubjects = [...subjects, newSubjectData];
      }
  ...
  const handleEditSubject = () => {
    ...
    setSelectedDays(selectedSubject.schedule.days || []);
    // Set the time values from the existing subject
    const [startTimeStr, endTimeStr] = selectedSubject.schedule.time.split(' - ');
    const [startHours, startMinutes, startPeriod] = startTimeStr.split(/[: ]/);
    const [endHours, endMinutes, endPeriod] = endTimeStr.split(/[: ]/);
  ...
  const renderSubjectOptionsModal = () => (
    <Modal
